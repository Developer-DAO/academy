...
are heavily decentralised, it makes this all possible. And there are many more
benefits to this decentralised model as we’ll continue to see.
...
functions below our state variable definitions. Do not delete our messageand
...
transaction logs.…. **Definition of parameter here….or in the Indexed Events
Parameter dropdown.**
...
There’s a lot to unpack, so we’ll leave it there for now! Let’s create an event
to log all the future messages sent to our smart contract and check the fortunes
to see if the sent messages are worthy of “making it” in the web3 world. 😉
...
addresses of other **smart contracts**. Yes, that is also for another lesson 😉
...
compare two strings of text, so with a little bit of encoding and decoding, we
are using abi.encodePacked() to convert the string into bytes and then using the
...
of our contract. Click it to open it.ons with the green check mark. The first
transaction is the deployment of our contract. Click it to open it.
