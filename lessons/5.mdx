...
When our page is loading, the `useEffect` hook runs to change the state of
`showMinter` to `true`. If the state remains `false` then nothing is returned.
If the state is `true` but the window object is undefined then an empty `jsx` 
element is returned.
