...
A CLI is actually quite straightforward tool to use, but it's a bit tricky in the beginning to get used to. You can't see everything readily, like in your computer's GUI interface **(?????)**, so you need to learn to **navigate** inside it and a bit of practice is needed. We have all had that little bit of frustration. Therefore this tutorial of the basics. 

CLI stands for **C**ommand **L**ine **I**nterface. Generally speaking, **I think**, Linux users call a CLI a terminal, Mac users call it a console, Windows users a shell. All these names have some technical differences, and we are going to stick with CLI.

**Why are we using this primitive looking stuff in the 21st Century?**

Come on, it looks retro, so it must be cool! By using text commands, a CLI lets us right into the **root** of our commuter's file system i.e. its operating system, which is something very powerful, and extreme care is needed. We can also access a world of open source work (repositories, libraries, etc) that our fellow developers have created before us. It’s acts like a gateway for us to collaborate through our machine to develop with the outside world. And that's what makes it cool. 

There are a few concepts we first need to understand before we start. A CLI is really old school, but it’s tried and tested. Still wondering? Using text is extremely resource efficient. No graphics. Text for input, text for output. For every task in the CLI, there’s a user command/programme for it. Each command is **wrapped in the shell** programme. This shell is an interpreter for the commands to the kernel (the operating system), which in turn is the gateway to all our hardware.
...
## CLI for Mac
![Linux-CLI-commands.png](/public/assets/lessons/CLI/Linux-CLI-commands.png)
...
Right, now that we are all on the same Linux page, let's open a CLI up and have a look. From your keyboard type the following and wait for the magic. For Linux press CTRL+ALT+T
Mac : **????????????????**
