...
In this lesson, we'll be adding automated tests for our smart contract that was
create in Lesson 3.
...
- help you write code that is testable
...
This is where automated test come in. Tests are a tool for writing your code,
...
Assuming you already have hardhat running in your project (from Lesson 2), add a
line in your `scripts` section of `package.json` so it looks something like
this:
...
many examples here in this lesson.
...
failing since there is no test code written. Adding `pending` placeholders are a
good way to brainstorm and remind yourself what tests you need to write.
...
the heavy lifting is done by OpenZeppelin base classes, but let's make sure we
can set the NFT name and symbol.

Let's take a leap forward with our first two tests!

It's looks like a lot, but we'll walk through. This code includes setup code
...
Next is a `beforeEach()` block. When tests run it's important that the contract
is deployed from scratch for each test and this is where that happens. "Before
each" test, this code will run and re-inialize everything, which prevents one
test's results from interfering with another test.
...
This group of tests introduce some new concepts that are very useful.
...
line. The contract call is temporarily removed here to highlight what's
happening. We're expecting the call to be reverted with that specific error
message from the contract when not sending enough payment.

The method inside that `expect()` method looks like this:
...
Now for the remaining mint tests, we want to successfully mint, so they all need
to mint with a minimum of `0.01` Eth. You'll see the same line at the top of the
remaining tests in this section.

And really each of those remaining tests all has a single `expect()` line and
test the various conditions upon a successful mint. The tier tests each put in
