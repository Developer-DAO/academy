...
![1_diagram.png](/public/assets/lessons/3/1_diagram.png)
...
<br/>
<details>
<summary>**Our code will look something like this**</summary>
```solidity
// SPDX-License-Identifier: UNLICENSED
pragma solidity 0.8.11;
...
We have added three types of tiers such as `NOOB` , `DEV` and `HACKER` . We
...
Now we need to add our mint function for the tier NFTs:
...
string constant TIER_NAME_0 = "NOOB"; string constant TIER_NAME_1 = "DEV";
string constant TIER_NAME_2 = "HACKER"; uint256 constant TIER_VALUE_0 = 0.01
...
Here, we prepared the start and end of our SVG. We can test this out by pasting
this code into this [online svg editor](https://editsvgcode.com/).
...
We are adding the RPC and the network to our config file. We will need some
Testnet to deploy the contract.

<br/>
<details>
<summary>**Details on adding the network and getting some testnet**</summary>

  ![2_faucet.png](/public/assets/lessons/3/2_faucet.png)
  To add the Polygon Mumbai network head over to the
  [Chainlist](https://chainlist.org/) and connect your wallet. Then toggle the
  testnet button and search for Mumbai. You will see the testnet network with
  chainID 80001 and add it to the wallet. Note: Always make sure to use a spare
  wallet with no money for these tutorials.
  ...
  some testnet on the wallets. Note: Testnet and mainnet are separate networks.
  You can not send money from testnet to mainnet. To get some testnet let’s head
  over to this [website](https://faucet.paradigm.xyz/).
  ![3_faucet.png](/public/assets/lessons/3/3_faucet.png)
  If you complete the required tasks that was asked, you can get testnets that
...
![4_deploy.png](/public/assets/lessons/3/4_deploy.png)
...
- `let txn = await [contract.mint](http://contract.mint)` is calling the mint
  function.
...
![5_mint.png](/public/assets/lessons/3/5_mint.png)

We finally minted our tier NFTs.

Let’s go ahead and view them on Opensea marketplace. This could take a few
...
![6_opensea.png](/public/assets/lessons/3/6_opensea.png)
