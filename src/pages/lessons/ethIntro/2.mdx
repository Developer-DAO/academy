...
Install web3.py, eth-tester, and IPython (if you didn't already in Part 1):
...
Account generation
Let's create that account:In [2]: w3 = Web3() # No provider necessary for this demo
...
Transferring ether
Recall that EthereumTesterProvider enables a test environment seeded with accounts and fake ether. Let's start by viewing some test accounts and an account balance:In [1]: w3 = Web3(Web3.EthereumTesterProvider())
...
Next, we'll introduce a new account:In [5]: acct_two = w3.eth.account.create()
...
Since this is the first transaction being made by acct_two, its nonce is zero. If you supply the wrong value, the result is an invalid transaction and is rejected by web3.py:ValidationError: Invalid transaction nonce: Expected 0, but got 4
...
Briefly, smart contracts are programs that live on the Ethereum blockchain, available for anyone to use. When you're ready to deploy a smart contract, you compile the code down to bytecode and include it in a transaction as a data value:bytecode = "6080604052348015610...36f6c63430006010033"
...
Note that tools like web3.py offer more intuitive interfaces for deploying and interacting with contracts:# interact with an existing contract:
...
We'll save a deep dive into message signing for a future post, but here's some pseudocode to give you an idea of the workflow:# 1. write a message
