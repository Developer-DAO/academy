...
<Layout
    ...
    description="Learn to seamlessly integrate your smart contracts with a user friendly front-end interface utilising web3 libraries. With engaging quizzes along the way." 
>
<LessonHeader title="Lesson 5: Connecting to a Front-end" />
...
timer ⌛ in the menu header. It'll remind you to "go touch grass" and appreciate
nature's role in enhancing our well-being. 🌱 Relish your journey in development
in a good state of mind and body.

## What are we building

{/* Major repetition going on with this next paragraph and the intro paragraph */}
...
use all of this knowledge and connect it to a front-end for users to see and
interact with.~~
...
public goods. We love open source at Developer_DAO. And public goods are gooood!
...
{/* @piablo check the descriptions below. I believe Marcos updated them with better versions */}
...
- [viem](https://viem.sh/docs/getting-started.html) is a compact JavaScript
  library for interacting with the Ethereum eco-system
- [WAGMI](https://wagmi.sh/) for checking connection to a wallet and calling
  functions from our smart contract through the front-end using the provided
  _hooks_.
...
- WALLETCONNECT STUFF: self-document steps to set-up from [here](https://www.rainbowkit.com/docs/installation) and choose place in lesson for these steps
...
`create-next-app` command uses the `yarn` package manager by default, but since
we have been using `npm` as the package manager for all our previous builds in
this track, we'll add the `--use-npm` flags:
...
n.b. see below in following code block // `import { useEffect, useState } from "react";` MARKKOS' NOTE: I would remove this `import` and consider using/moving it to `index.js` imports explanation - go to line :238 and see comments re: sidedrawer needed
```jsx
import "@/styles/globals.css";
...
- SIDE-DRAWER NEEDED FOR THESE: `useEffect` and `useState` are react hooks that help us perform side effects
...
MAYBE HERE FOR WALLETCONNECT??: Because our dApp is going to use WalletConnect, we need to first go an grab a `projectID`
from the [WalletConnectCloud](https://cloud.walletconnect.com/sign-in). You'll be done in a few minutes 🤗
Let's get back to the `_app.js` file and create the configurations we need, and create a new instance
of `wagmiClient` which uses them:
...
Great! Now we can wrap our application in the `RainbowKitProvider` and
`WagmiConfig` so that it can have access to their features throughout our
application. Our code should look like this:
...
`index.js` page in the `/frontend/pages` directory. This file work as our
homepage at the root level, in our url will be `htpp://localhost:3000/` by
default and it's code should look like this after deleting all the _children_
within the `<main>` element and completely deleting the `<footer>`.
...
(add more explanations maybe explaining how they work together and why we need
it to perform better the UX)
...
- A nextJS `Image` element renders a preview of the NFT Image. Earlier, we saved
  ...
  explicitly.
- A `button` element let's us first set the minting price choosen and then it
...
export default function Home() {
  const CONTRACT_ADDRESS =
    ...
    "0x5794c3de7a59587048b76060d282476e12a8aff8";
  ...
  useEffect(() => {
    try {
      if (tokenURI) {
        setLatestNFTMinted(
          JSON.parse(
            window.atob(tokenURI.substring(tokenURI.indexOf(",") + 1)),
          ),
        );
      }
    } catch (error) {
