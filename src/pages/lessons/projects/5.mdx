...
{/* upgrades to descriptions are welcome. I/we might need to move the actual URLs down into 
the tutorial if really needed. For now, they avoid that potential rabbit hole scenario while here */}
...
Because our dApp is going to use WalletConnect, we need a `projectID`
from the [WalletConnectCloud](https://cloud.walletconnect.com/sign-in). 
So go and set up an account and grab that ID. You'll be done in a few minutes 🤗

All good? Okay, let's get back to the `_app.js` file, set up the configurations we need, and create a new instance
of the `wagmiClient` which will use them:
...
// function Home() code...
```
{/* if we can avoid the  following link, that would be great. I think at a later stage 
 after this MVP, we can come back and create a few side-drawers for these hooks: */}
