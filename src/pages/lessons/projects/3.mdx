...
<NextSeo
  ...
  description="Write and deploy a Smart Contract for a Tier Based NFT. Builds on knowledge from Lesson 2."
/>
<LessonHeader
  title="Lesson 3: Tier NFTs"
...
pomodoro timer ⌛ in the menu header to help you pace yourself and remind you to
"touch grass" and appreciate nature's contribution to our well-being. 🌱
...
Fire up your code editor and let’s start hacking. I’m using VSCode, so I just
...
Now let’s go ahead and add a mint function which will call `_safeMint` from the
...
For this lesson we won’t be creating a separate JSON file. We will actually code
...
- `require(success, "Withdraw failed")` - This is a good practice because the
  `.call()` function doesn’t revert. With this practice we can make sure that
  the transfer occurred and throw an error if it doesn’t.
